...
const CollectionImagesPreview = ({images}:{images:string[]}) => {
  ...
  return (
      <div className="w-full h-fit flex flex-col gap-4 sticky top-10">
        <RoundedImage src={images[selectedImage]} alt='anjay' className='!w-full !aspect-[4/3] !rounded-lg' />
        <div className='w-full flex items-center gap-5 overflow-x-auto pb-4 !box-border'>
          {images.map((image, i) => (
            ...
            >
              <RoundedImage src={image} alt={image} className={`!w-full !aspect-[4/3] !rounded-md`} />
            </button>
          ))} 
        </div>  
      </div>
