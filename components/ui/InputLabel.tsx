export default function InputLabel({ forInput, value, className,textarea=false }:{forInput:string,value: string,className?:string,textarea?:boolean}) {
    return (
        <label 
        htmlFor={forInput} 
        className={`absolute text-sm text-semiDark duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white px-2 peer-focus:px-2 
        peer-focus:text-gray-700 peer-placeholder-shown:scale-100 
